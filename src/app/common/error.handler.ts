...
export class ErrorHandler {
        ...
        if (formField) {
            ...
            if (this.hasError(formField)) {
                if (errors.maxlength) {
                    ...
                    message = errors.serverError[0].message ? errors.serverError[0].message : '';
                } else {
                    message = '';
                }
            }
    ...
    setError(error: any, form: FormGroup) {
        ...
        if (errors && form && errors.length === undefined) {
            Object.keys(errors).forEach(field => {
                const isNestedField = errors[field].length === undefined;  // field is not an array
                if (isNestedField) {
                    this.setErrorsToNestedField(form, errors[field], field);
                } else {
